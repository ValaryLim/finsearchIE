'''
Processes data_zhiren.csv file into format accepted by Prodigy.
'''
import json
import pandas as pd
...
if __name__ == "__main__":
    data_zhiren = pd.read_csv('data/external/external_zhiren.csv')

    # represent each unique article in the format {'text': article, 'meta': {'id': article_id}}
    prev_article = ''
    curr_id = 0
    articles = []
    for ind, row in data_zhiren.iterrows():
