...
def macaw_processing(jsonl_path):
    ...
    for abstract in df:
        ...
        for i in range(len(abstract['sentences'])): 
            sentence, ner_list, rel_list = abstract['sentences'][i], abstract['ner'][i], abstract['relations'][i]
            sentence_text = ' '.join(sentence)
            # generate ner pairs
            ner_list = [' '.join(abstract_text_list[x[0]:x[1]+1]) for x in ner_list] # clean ner_list
            ...
            comb_ner_pairs = [[pair[0], pair[1], identify_relation(pair, rel_dict)] for pair in comb_ner_pairs]
            perm_ner_pairs = [[pair[0], pair[1], identify_relation(pair, rel_dict, comb=False)] for pair in perm_ner_pairs]
            # update relation dicts
            for comb_pair in comb_ner_pairs:
                comb_all.append({
                    'E1': comb_pair[0], 'E2': comb_pair[1], 'R': comb_pair[2], 'sentence': sentence_text, 
                    'abstract': abstract_text, 'doc_key': abstract_key
                })
            for perm_pair in perm_ner_pairs:
                perm_all.append({
                    'E1': perm_pair[0], 'E2': perm_pair[1], 'R': perm_pair[2], 'sentence': sentence_text, 
                    'abstract': abstract_text, 'doc_key': abstract_key
                })
            # update ner list
        ...
        for pair, pair_coref in coref_dict.items():
            coref_all.append({
                'E1': pair[0], 'E2': pair[1], 'C': pair_coref, 'abstract': abstract_text, 'doc_key': abstract_key  
            })
...
if __name__ == "__main__":
    source_path = "data/prodigy_processed/"
    destination_path = "data/macaw/macaw_gold/"
